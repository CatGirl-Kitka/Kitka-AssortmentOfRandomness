...
public class KitkaExfilAssortment_TradersLoader(
    ModHelper modHelper,
    ...
    : IOnLoad
{
    ...
    public Task OnLoad()
    {
        ...
        var traderBase = modHelper.GetJsonDataFromFile<TraderBase>(pathToMod, "db/TraderKitka/base.json");
        ...
        addCustomTraderHelper.AddTraderToLocales(traderBase, "Kitka", "A strange woman with cat ears and a cat's tail, she mostly sells strange, useless things, or things that nobody wants. Where does she get all these items? Nobody knows, but one thing is certain: nobody wants them except collectors and strange people");

        // Get the assort data from JSON
        var assort = modHelper.GetJsonDataFromFile<TraderAssort>(pathToMod, "db/TraderKitka/assort.json");
